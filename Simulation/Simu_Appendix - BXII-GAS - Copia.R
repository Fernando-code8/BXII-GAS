# Reference: UG-GAS
# Created by Fernando J M de Araújo (nandomonteiro418@gmail.com), February/2024
rm(list=ls())

setwd("C:/Users/nando/OneDrive/Desktop/mestrado UFRGS/Dissertação/UG-GAS e BXII-GAS 20241120-20241217T004454Z-001/UG-GAS e BXII-GAS 20241120/Scripts Atual BXII-GAS/BXII-GAS")
...
for(j in 1:length(vn)){
  ...
  while(i<=R){
    print(c(i,j))
    y <- simu.BXIIGAS(n,A=A,B=B,w=w,c=c,beta=beta,tau=0.75,X=cov)
    # y <- simu.BXIIGAS(n,A=A,B=B,w=w,c=c,tau=0.75)
    result <- try(BXIIGAS.fit(y,ar=1,ma=1,X=X,X_hat=X_hat,tau=0.75,
    ...
    if(class(result)=="try-error" || 
      ...
      if (ICi[i,6,j]<= theta[6] && ICs[i,6,j]>=theta[6])
      {
        cc<-cc+1
      }
      i<-i+1
  ...
  TC[j,]<- c(comega,calpha1,cbeta1,cdelta1,cdelta2,cc)/R

  }
scen=1

setwd("C:/Users/nando/OneDrive/Desktop/mestrado UFRGS/Dissertação/UG-GAS e BXII-GAS 20241120-20241217T004454Z-001/UG-GAS e BXII-GAS 20241120/Scripts Atual BXII-GAS/Simulation")
saving<-paste0("results_simu/simu_scensin0.75",scen,"R",R,".RData")
save.image(saving)
end_time <- Sys.time()
