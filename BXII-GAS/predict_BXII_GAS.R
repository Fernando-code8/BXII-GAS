################################################################################
# BXII-GAS - Forecast one-step-ahead approach
################################################################################

predict_BXIIGAS<-function(fit_BXIIGAS,y_test,X=X,X_hat=X_hat,tau=0.5,link="log"){
  ...
  omega<-fit_BXIIGAS$coeff[1]
  A<-fit_BXIIGAS$A
  B<-fit_BXIIGAS$B
  c<-fit_BXIIGAS$c
  y_train<-fit_BXIIGAS$serie
  n<-length(y_train)
  h1<-length(y_test)
  #### Forecasting
  ar<-1:length(A)
  ma<-1:length(B)
  namesA <- names(fit_BXIIGAS$coeff[1+ar]);orderA <- as.numeric(gsub("A", "", namesA))
  namesB <- names(fit_BXIIGAS$coeff[1+length(A)+ma]);orderB <- as.numeric(gsub("B", "", namesB))
  ...
  if(k==0)  beta <- as.matrix(0) else beta<-fit_BXIIGAS$beta
  ...
  muhat<- c(fit_BXIIGAS$fitted[(n-m):(n-1)],rep(NA,h1))
  sthat<- c(fit_BXIIGAS$sthat[(n-m):(n-1)],rep(NA,h1))
  fthat<- c(fit_BXIIGAS$fhat[(n-m):(n-1)],rep(NA,h1))
  ynew_prev <- c()
  ...
  for(i in 1:h1){
    ...
    sthat[m+i] <- st.funcBXII(muhat[m+i],y_t[m+i],c=c,tau=tau,link=link)
  }
  return(muhat[-c(1:m)])
}